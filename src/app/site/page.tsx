...
export default async function WorkspacePublicPresencePage() {
  ...
  if (!pageData) {
    return (
      <div style={{ height: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: '1rem', backgroundColor: '#0F1112', color: '#E3E4E6' }}>
        <h1 style={{ fontSize: '2rem', color: siteSettings?.brand?.colors?.[0]?.value || '#B9783B' }}>Site Not Published Yet</h1>
        <p>This workspace public presence does not have a homepage configuration setup.</p>
      </div>
    );
  }
