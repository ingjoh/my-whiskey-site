  ...
  switch (node.type) {
    ...
    case 'Image':
      Content = node.props.src ? (
        ...
        <div style={{ ...node.props.style, width: '100%', minHeight: '200px', backgroundColor: 'var(--color-background)', border: '2px dashed var(--color-border)' }} />
      );
      break;
    case 'Button':
      Content = (
        <button style={{ ...node.props.style, border: 'none', cursor: 'pointer', padding: node.props.style?.padding || '0.75rem 1.5rem', background: node.props.style?.background || 'var(--color-primary)', color: node.props.style?.color || 'white', borderRadius: node.props.style?.borderRadius || 'var(--radius-sm)' }}>
          {node.props.text}
        </button>
      );
      break;
...
export default async function DynamicPublicPage({ params }: { params: Promise<{ pageId: string }> }) {
  const { pageId } = await params;
  if (pageId.startsWith('template-')) {
    notFound();
  }
                ...
                {publishedItems.map((item) => (
                  ...
                  >
                    ...
                    <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', justifyItems: 'space-between', flex: 1 }}>
                      <div>
                        {/* Render Subtitles depending on Type */}
